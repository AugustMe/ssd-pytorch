...
if __name__ == "__main__":
    ...
    model = get_ssd("train", Config["num_classes"]) # 创建模型
    ...
    criterion = MultiBoxLoss(Config['num_classes'], 0.5, True, 0, True, 3, 0.5, False, Cuda) # 损失函数

    net = model.train()
    if Cuda:
        # net = torch.nn.DataParallel(net) # 多GPU训练
        net = torch.nn.DataParallel(model, device_ids=[0])  # 多卡训练出错，还未解决
    ...
    if True:
        ...
        lr_scheduler = optim.lr_scheduler.StepLR(optimizer,step_size=1,gamma=0.92) # 学习率
        ...
        if Use_Data_Loader:
            ...
            gen_val = DataLoader(val_dataset, shuffle=True, batch_size=Batch_size, num_workers=16, pin_memory=True,
                                 drop_last=True, collate_fn=ssd_dataset_collate)
        else:
        ...
        #   冻结后训练
        #------------------------------------#
        # for param in model.vgg.parameters(): # model被torch.nn.DataParallel()包裹，不应该是model
    ...
    if True:
        ...
        #   解冻后训练
        #------------------------------------#
        # for param in model.vgg.parameters(): # model被torch.nn.DataParallel()包裹，不应该是model，而是net
        for param in net.module.vgg.parameters():
